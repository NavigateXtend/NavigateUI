  ...
  return (
    <div className="sidebar backdrop-blur[1px] fixed z-[90] w-fit select-none overflow-y-scroll pb-7 xl:px-5">
      <div className="group flex h-full w-full flex-col space-y-6">
        {/* container   */}
        <div className="text- flex flex-col pt-10 font-sans text-gray-600 duration-300 xl:px-4">
          {/* Docs  */}
          <div onClick={() => setDocIsOpen(!isDocOpen)} className="flex cursor-pointer items-center justify-between rounded-lg px-6 py-2 text-white duration-300">
          ...
          <div className={`grid text-slate-600  transition-all duration-300 ease-in-out ${isDocOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
            <div className="my-2 ml-8 flex flex-col space-y-2 overflow-hidden border-sky-400  px-6 text-lg ">
              <Link className="px-6  py-2 text-white duration-300 hover:rounded-lg hover:bg-slate-500 hover:text-white" href="/">
                Introduction
              </Link>
              <Link className="px-6 py-2 text-white duration-300 hover:rounded-lg hover:bg-slate-500 hover:text-white" href="/Components/avatar">
                Quick Start
              </Link>
            </div>
          </div>
          {/* Component  */}
          <div onClick={() => setComponentIsOpen(!isComponentOpen)} className="flex  cursor-pointer items-center justify-between rounded-lg px-6 py-2 text-white">
            <div className="flex items-center gap-4">
              <svg style={{ filter: 'b' }} className="drop-shadow-[0px_0px_10px_#0ea5e9]" width={25} fill="#0095FF" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                <g id="SVGRepo_iconCarrier">
                  <title>shapes</title>
          ...
          <div className={`grid  text-slate-600 transition-all duration-300 ease-in-out ${isComponentOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
            <div className="my-2 ml-8 flex flex-col space-y-2 overflow-hidden  border-sky-400 px-6 text-lg">
              {navComponentsItems.map((componentName, i) => (
                <Link
                  key={i}
                  className={`rounded-lg ${pathname === componentName.toLowerCase().replace(/\s+/g, '') ? ' border-sky-500 text-white' : 'border-transparent text-white hover:bg-slate-500 '} rounded-md border  px-6 py-2 duration-300
                                    `}
