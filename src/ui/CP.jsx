  ...
  return (
    <div className={`my-5 h-fit w-full min-w-80 max-w-full`}>
      <div className="item-center mb-2 flex justify-between gap-2">
        <div className={`item-center ${tabNum === 1 ? 'border-sky-500' : 'border-slate-800 dark:border-slate-400/20'} relative flex overflow-hidden rounded-sm border-2 duration-300`}>
          <div
            className={`absolute z-40 flex h-9 w-20 items-center ${tabNum === 1 ? 'bg-sky-500' : 'bg-slate-800 dark:bg-slate-400/20'}  duration-300`}
      ...
      <div className={`relative w-full rounded-sm`}>
        <div className={`absolute inset-0 -z-50 rounded-sm backdrop-blur-lg ${tabNum === 1 ? 'bg-slate-800 dark:bg-slate-400/20' : ' border border-slate-800 dark:border-slate-400/10'}`}></div>
