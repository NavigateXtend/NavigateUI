...
export const Modal2 = () => {
    ...
    return (
      <div>
        <button onClick={() => setOpenModal(true)} className="rounded-sm bg-sky-500 px-5 py-[6px] text-white">Open\</button>
        <div className={\`fixed z-[100] flex items-center justify-center \${openModal ? 'opacity-1 visible' : 'invisible opacity-0'} inset-0 bg-black/20 backdrop-blur-sm duration-100\`}>
          <div className={\`absolute max-w-md rounded-sm bg-white p-3 pb-5 text-center drop-shadow-2xl \${openModal ? 'scale-1 opacity-1 duration-300' : 'scale-0 opacity-0 duration-150'} \`}>
            ...
            <h1 className="mb-2 text-xl font-semibold">Welcome to NavigateUI!</h1>
            ...
            <button onClick={() => setOpenModal(false)} className="me-2 rounded-sm bg-green-700 px-6 py-1 text-white">Ok</button>
          </div>
        </div>
      </div>
    )}`;
  ...
  return (
    <CP code={code}>
      <div className="mx-auto w-fit">
        ...
        <div className={`fixed z-[100] flex items-center justify-center ${openModal ? 'opacity-1 visible' : 'invisible opacity-0'} inset-0 bg-black/20 backdrop-blur-sm duration-100`}>
          <div className={`absolute max-w-md rounded-sm bg-white p-3 pb-5 text-center drop-shadow-2xl ${openModal ? 'scale-1 opacity-1 duration-300' : 'scale-0 opacity-0 duration-150'} `}>
            <svg onClick={() => setOpenModal(false)} className="mx-auto mr-0 w-8 cursor-pointer" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g strokeWidth="0"></g>
            ...
            <h1 className="mb-2 text-xl font-semibold">Welcome to NavigateUI!</h1>
            <p className="mb-3 px-1 text-sm opacity-80">Elevate your React projects with beautifully crafted components designed for TailwindCSS.</p>
            <button onClick={() => setOpenModal(false)} className="me-2 rounded-sm bg-green-700 px-6 py-1 text-white">
              Ok
            </button>
          </div>
        </div>
