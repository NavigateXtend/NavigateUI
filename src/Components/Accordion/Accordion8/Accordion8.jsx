                ...
                {accordionData.map((_, idx) => (
                    <div key={idx} className="flex">
                        ...
                        <button onClick={() => handleToggle(idx)} className={\`flex h-full w-fit items-end border-b-8 bg-white pb-10 \${_.colorBorder} shadow-[2px_2px_5px_#00000083]\`}>
                            <div className={\`h-16 w-16 text-white \${_.colorBg} relative flex items-center justify-center\`}><span className={\`h-0 w-0 \${_.colorBorder} absolute left-8 -z-50 rotate-[225deg] rounded-lg border-b-[60px] border-r-[60px] border-r-transparent\`}></span>0{idx + 1}</div>
                        </button>
                        {/* container */}
