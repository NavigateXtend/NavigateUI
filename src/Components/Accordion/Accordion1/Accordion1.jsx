...
export default function Accordion1() {
  ...
  return (
    <CodeBox codeStr={codeStr}>
      <div className="mx-4 rounded-lg border bg-black font-sans dark:bg-sky-200">
        {accordions.map((PerAccordion, idx) => (
          <div key={idx} className="border-b p-4 dark:border-black">
            ...
            <div
              className={`grid overflow-hidden text-gray-400 transition-all duration-300 ease-in-out dark:text-gray-800 ${isOpen === idx ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}
            >
              <div className="overflow-hidden">{PerAccordion.description}</div>
            </div>
          </div>
