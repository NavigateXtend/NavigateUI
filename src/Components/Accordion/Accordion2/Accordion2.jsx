...
const codeStr =`export const Accordion2 = () => {
  const datas = [
    ...
    return (
        <CodeBox codeStr={codeStr}>
            <div className=" bg-[#18181B] rounded-lg mx-4 lg:mx-8">
                ...
                {datas.map((data, idx) => (
                    <div key={idx}>
                        <div onClick={() => handleToggle(idx)} className={`${idx === data.length - 1 ? 'border-none' : 'border-b border-gray-400/10'} py-4 px-6  flex items-center gap-4`}>
                            <div>{data?.svg}</div>
                            <div className="flex-1">
                                <p className="font-medium text-lg text-white">{data?.title}</p>
                                <p className="text-gray-400">
