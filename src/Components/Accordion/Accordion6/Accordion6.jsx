...
const codeStr = `const Accordion6 = () => {
    const datas = [
        {title: 'Connected Devices',subTitle: '4 unread messages',coloredText: '',description: 'To create an account, click on the "Sign Up" button and fill out the required information. Once done, you can enjoy the benefits of being a registered member.'},
        {title: 'Apps Permissions',subTitle: '3 incompleted steps',coloredText: '',description: 'Our return policy allows you to return items within 30 days of purchase. Please visit our returns page for detailed instructions and to initiate a return.'},
        {title: 'Pending task',subTitle: '2 issue to',coloredText: 'fix now',description: 'Yes, you can change your shipping address before your order is shipped. Go to your account settings and update the shipping information accordingly.'},
    ];
    ...
    return (
        <div className=" mx-4 rounded-lg bg-[#18181B] lg:mx-8">
            ...
            {datas.map((data, idx) => (
                <div key={idx}>
                    <div onClick={() => handleToggle(idx)} className={\`\${idx === data.length - 1 ? 'border-none' : 'border-b border-gray-400/10'} flex  items-center gap-4 p-6\`}>
                        <div>
                            <img className={\`h-14 w-14 rounded-2xl border-2 bg-[#18181B] p-0.5 \${idx === 0 ? 'border-green-500' : idx === 1 ? 'border-yellow-500' : idx === 2 ? 'border-[#0095FF]' : 'border-white'}\`} src={\`https://source.unsplash.com/600x600/?profile/\${idx}\`} alt=""/>
                        </div>
                        <div className="flex-1">
                            <p className="text-lg font-medium text-white">{data?.title}</p>
                            <p className="text-gray-400">{data?.subTitle} {/*give color based on condition */}
                ...
                {datas.map((data, idx) => (
                    <div key={idx}>
                        <div onClick={() => handleToggle(idx)} className={`${idx === data.length - 1 ? 'border-none' : 'border-b border-gray-400/10'} flex  items-center gap-4 p-6`}>
                            <div>
                                <img
                                    className={`h-14 w-14 rounded-2xl border-2 bg-[#18181B] p-0.5 ${
