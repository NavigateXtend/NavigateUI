    ...
    return (
        <div className="space-y-4 p-2 md:p-6">
            {datas?.map((data, idx) => (
                <div key={idx}>
                    {/* header / Title */}
                    <div onClick={() => handleToggle(idx)} className={\`px-4 py-6 md:px-8 \${ idx === 0 ? 'border-green-500 bg-green-50' : idx === 1 ? 'border-sky-500 bg-sky-50' : idx === 2 ? 'border-purple-500 bg-purple-50' : idx === 3 ? 'border-amber-500 bg-amber-50' : idx === 4 ? 'border-red-500 bg-red-50' : 'border-orange-500 bg-orange-50'} cursor-pointer border-l-[3px]\`}>
                        <div className="flex items-center">
                            <span><svg className={\`mr-4 \${ idx === 0 ? 'fill-green-900' : idx === 1 ? 'fill-sky-900' : idx === 2 ? 'fill-purple-900' : idx === 3 ? 'fill-amber-900' : idx === 4 ? 'fill-red-900' : 'fill-orange-900'} shrink-0\`} width="16" height="16" xmlns="http://www.w3.org/2000/svg"><rect y="7" width="16" height="2" rx="1" className={\`origin-center transform transition duration-200 ease-out \${isOpen === idx && '!rotate-180'}\`}/><rect y="7" width="16" height="2" rx="1" className={\`origin-center rotate-90 transform transition duration-200 ease-out \${isOpen === idx && '!rotate-180'}\`}/></svg></span>
                            <h4 className={\`\${ idx === 0 ? 'text-green-900' : idx === 1 ? 'text-sky-900' : idx === 2 ? 'text-purple-900' : idx === 3 ? 'text-amber-900' : idx === 4 ? 'text-red-900' : 'text-orange-900'} text-xl\`}>
                                {data.title}
                            </h4>
                        </div>
                    </div>
                    ...
                    <div className={\`grid overflow-hidden transition-all duration-300 ease-in-out \${isOpen === idx ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}\`}>
                        <div className="overflow-hidden">
                            <div className={\`border-l-[3px] pb-6 pl-14 pr-4 text-sm md:pl-16 \${ idx === 0 ? 'border-green-500 bg-green-50 text-green-900' : idx === 1 ? 'border-sky-500 bg-sky-50 text-sky-900' : idx === 2 ? 'border-purple-500 bg-purple-50 text-purple-900' : idx === 3 ? 'border-amber-500 bg-amber-50 text-amber-900' : idx === 4 ? 'border-red-500 bg-red-50 text-red-900' : 'border-orange-500 bg-orange-50 text-orange-900'}\`}>
                                {data?.description}
                            </div>
                        </div>
                    </div>
