            ...
            <div className="mx-auto flex min-h-[200px] w-full bg-sky-400 md:min-h-[300px]">
                {accordionData.map((_, idx) => (
                    <div key={idx} className="flex">
                        {/* toggle item */}
                        <button onClick={() => handleToggle(idx)} className={\`flex h-full w-6 items-center justify-center bg-sky-500 lg:w-14 \${isActive === idx && 'bg-lime-600'} relative z-20 text-white shadow-[2px_2px_5px_#00000083]\`}><p>{idx + 1}</p></button>
                        {/* container */}
